...
def plot_communities(G, node_community_labels, title="Communities", details=True, metrics=None):
    if not details:
        args = {
            'width': 0.5,
            'node_size': 50}
    ...
    nx.draw(G, pos, node_color=node_community_labels, with_labels=details, cmap=plt.cm.Set3, **args)
    plt.title(title)
